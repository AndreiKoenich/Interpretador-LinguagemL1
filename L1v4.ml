...
let rec eval (renv:renv) (e:expr) (mem:mem) : valorMem =
  ...
  | Dref(e) ->
      ...
      (match fst v with
         VNum(n) -> (lookup_mem (snd v) n, (snd v))
...
let testeA = New(Num 3301)
let testeB = New(True)
let testeC = Whl(True,Skip)
let testeD = Dref(testeA)
let testeE = Dref(testeB)
let testeF = Seq(Skip,Num 3301)
let testeG = Seq(Skip,True)
let testeH = Asg(testeA,Num 42)
let testeI = Asg(testeB,True)
...
let teste0 = Dref(New (Num 54)) 
    
    (* Casos de teste disponiveis no moodle *)
